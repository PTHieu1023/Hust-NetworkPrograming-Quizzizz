...
export const changePassword = createAsyncThunk(
    'auth/changePassword',
    async (data: { old: string; new: string }, thunkAPI) => {
        try {
            const message = await userService.changePassword(data.old, data.new)
            notify(message as string, 'success')
        } catch (error: any) {
            return thunkAPI.rejectWithValue(
                error?.err || 'Password change failed'
            )
        }
    }
)
