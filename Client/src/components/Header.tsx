...
const Header: React.FC<{ tab: string; user: any }> = ({ tab = 'home', user }) => {
        ...
        <>
            <div className="fixed top-0 z-10 w-full shadow-md bg-base-100">
                <div className="navbar justify-between w-full">
                    ...
                    <div className="md:hidden uppercase text-lg text-primary font-bold ml-2">{tab}</div>
                    ...
                    <div className="flex justify-end gap-8">
                        <div className="md:flex items-center hidden">
                            ...
                            <Link
                                to={`${ROUTES.DASHBOARD_ROUTE}?tab=room`}
                                className={`flex gap-2 items-center justify-center px-4 pb-5 -mb-5 cursor-pointer hover:opacity-75 hover:text-primary ${tab == 'room' && 'text-primary border-b-2 border-primary'}`}
                            >
                        ...
                        <div className="relative flex gap-2 justify-end items-center">
                            ...
                            <div
                                className={`absolute top-full right-0 z-1 rounded-lg bg-base-100 w-48 shadow-md flex flex-col gap-2 p-4 transition-all origin-top-right duration-500 ease-in-out ${
                                    isMenuOpen ? 'opacity-100 scale-100 h-fit' : 'opacity-0 scale-50 pointer-events-none'
                                }`}
                            >
                                ...
                                <Link to={ROUTES.PROFILE_ROUTE} className="flex gap-2 items-center w-full p-2 hover:bg-black/5">
                                    <FontAwesomeIcon icon={faGear} />
                                    Setting
                                </Link>
                                <button className="flex gap-2 items-center w-full p-2 hover:bg-black/5" onClick={handleLogout}>
            ...
            <div className="md:hidden fixed z-20 bottom-12 w-full">
                <div className="mx-4 flex space-x-2 justify-around items-center rounded-full bg-base-100 shadow-lg h-16">
                    <Link to={`${ROUTES.DASHBOARD_ROUTE}?tab=home`} className="flex flex-col">
                        <FontAwesomeIcon icon={faHouseChimney} />
                        <span className="text-sm">Home</span>
                    </Link>
                    <Link to={`${ROUTES.DASHBOARD_ROUTE}?tab=search`} className="flex flex-col">
                        <FontAwesomeIcon icon={faMagnifyingGlass} />
                        <span className="text-sm">Search</span>
                    </Link>
                    <Link to={`${ROUTES.DASHBOARD_ROUTE}?tab=activity`} className="flex flex-col">
                        <FontAwesomeIcon icon={faClockRotateLeft} />
